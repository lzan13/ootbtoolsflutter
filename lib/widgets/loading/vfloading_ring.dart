...
class VFLRing extends StatefulWidget {
  const VFLRing({
    super.key,
    required this.color,
    this.lineWidth = 7.0,
  ...
  @override
  State<VFLRing> createState() => _VFLRingState();
}

class _VFLRingState extends State<VFLRing> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _animation3 = Tween(begin: 0.25, end: 5 / 6).animate(
      CurvedAnimation(
        parent: _controller,
        curve: const Interval(0.0, 1.0, curve: VFLRingCurve()),
      ),
    );
  }
...
class VFLRingCurve extends Curve {
  const VFLRingCurve();
