...
class OTLRing extends StatefulWidget {
  const OTLRing({
    super.key,
    required this.color,
    this.lineWidth = 7.0,
  ...
  @override
  State<OTLRing> createState() => _OTLRingState();
}

class _OTLRingState extends State<OTLRing> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _animation3 = Tween(begin: 0.25, end: 5 / 6).animate(
      CurvedAnimation(
        parent: _controller,
        curve: const Interval(0.0, 1.0, curve: OTLRingCurve()),
      ),
    );
  }
...
class OTLRingCurve extends Curve {
  const OTLRingCurve();
